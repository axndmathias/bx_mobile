...
* **ViewModels:** Verwalten den Zustand der Oberfläche mithilfe leichter Flutter-Primitiven (`ValueNotifier` / `ChangeNotifier`). Sie empfangen Eingaben aus den Views und führen die entsprechenden UseCases aus.
...
[ User Action ] ──> ( View )
                      │ (Benachrichtigt)
                      ▼
               ( ViewModel ) ──> [ Aktualisiert ValueNotifier ] ──> ( View Rebuild )
                      │
                      │ (Führt aus)
                      ▼
                 ( UseCase )
...
Die zentrale Verantwortung des Containers besteht darin, Authentifizierung, Navigation, Lizenzverwaltung, Dependency Injection und die Kommunikation zwischen den Modulen bereitzustellen, während die einzelnen BMX-Module ihre jeweilige Geschäftslogik vollständig kapseln.
